...
export async function run(client: Ryneczek, interaction: ChatInputCommandInteraction) {
	...
	const modalUse = await client.useModal(interaction, reportModal, client.ms('5m'));
	if(!modalUse) return;

	await modalUse.reply({ content: 'Zgłoszenie zostało wysłane!', ephemeral: true });
	...
	await (client.channels.cache.get(client.config.report_channel) as BaseGuildTextChannel)?.send({ embeds: [embed], components: [components] });
}
