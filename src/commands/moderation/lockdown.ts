...
export async function run(
	...
	const categories = interaction.guild.channels.cache
		...
		.filter(
			(category) =>
				!client.config.lockdown_ignored_categories.includes(category.id),
		);

	const isLocked = categories.some((category) => {
		return category.permissionOverwrites.cache
			.find(
				(overrite) =>
					overrite.type == OverwriteType.Role &&
					overrite.id === interaction.guild.roles.everyone.id,
			)
	...
	const embed = new EmbedBuilder()
		.setColor(
			isLocked ? ("Green" as ColorResolvable) : ("Red" as ColorResolvable),
		)
		.setDescription(
