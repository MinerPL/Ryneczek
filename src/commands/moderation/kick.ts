...
export async function run(client: Ryneczek, interaction: ContextMenuCommandInteraction) {
			...
			new ActionRowBuilder<ModalActionRowComponentBuilder>()
				.addComponents(
					new TextInputBuilder()
						...
						.setLabel('Powód')
						.setCustomId('reason')
						.setPlaceholder('Dlaczego chcesz wyrzucić tego użytkownika? Powód będzie dostępny publicznie.')
						.setMinLength(1)
						.setMaxLength(1000)
						.setRequired(true),
				),
	...
	const embed = new EmbedBuilder()
		.setColor('#87b55b')
		.setDescription(`[${new Date().getDate()}.${new Date().getMonth() + 1}.${new Date().getFullYear()}] **${member.user.tag}** (${member.user.id}) został wyrzucony za: **${reason}**`)
		.setFooter({
